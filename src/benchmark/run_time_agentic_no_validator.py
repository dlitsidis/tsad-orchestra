...
async def main():
    ...
    for table in ts_tables:
        print(f"Benchmarking agentic (no validator) execution time for table: {table}")
        ...
        if existing:
            print(f"  Skipping agentic (no validator) on {table}: Already computed.")
            continue

        try:
            start_time = time.perf_counter()
            _ = await run_agent(table, skip_validator=True)
            elapsed_time = time.perf_counter() - start_time
            
            print(f"  Finished agentic (no validator) in {elapsed_time:.4f} seconds.")
            ...
            print(f"  Failed agentic (no validator) on {table}: {e}")
