import os, requests, zipfile, random, pandas as pd
...
DB_URL = f"postgresql://{os.getenv('POSTGRES_USER')}:{os.getenv('POSTGRES_PASSWORD')}@{os.getenv('POSTGRES_HOST')}:{os.getenv('POSTGRES_PORT', '5432')}/{os.getenv('POSTGRES_DB')}"
    ...
    for i, file_path in enumerate(selected_files, 1):
        ...
        try:
            with engine.begin() as conn:
                ...
                conn.execute(text(f'CREATE TABLE IF NOT EXISTS "{tbl}" (time TIMESTAMPTZ NOT NULL, data DOUBLE PRECISION, label INTEGER)'))
                
                is_hyper = conn.execute(text("SELECT 1 FROM timescaledb_information.hypertables WHERE hypertable_name = :t"), {"t": tbl}).fetchone()
                if not is_hyper:
                ...
                df.to_sql(tbl, conn, if_exists='append', index=False, method='multi', chunksize=5000)
